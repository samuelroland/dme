...
#image("logo/logo.svg", height: 8em)
...
+ You could use an easier solution, the `Markdown PDF` extension in VSCode. It provides a simple `Export to PDF` action in the command palette. But what happens if you don't like the default theme ? You have to write pure CSS to change the style of headings, tables, line height, to remove this strange yellow, etc.
#figure(
...
#figure(
  ...
  caption: [Another preview of Java code in VSCode. Lots of parts in white are not colorized.],
...
#figure(
  image("imgs/grammars-management.png", width: 70%),
  caption: [],
...
#figure(
  image("imgs/preview-readme-colors.png", width: 70%),
  caption: [],
...
#figure(
  ...
  caption: [The same snippet of Java highlighted in DME],
