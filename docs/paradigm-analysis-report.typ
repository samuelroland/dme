...
Compilator first define the owner of each variable. That way, the owner will be able to drop the variable when it reached the end of its lifetime, essentially 
freeing the memory allocated to for the variable.

Behind the scene, when we allocate memory on the heap, the compilator will also determine when this memory will freed, adding in the source code the drop instruction.
While it is not directly visable, displaying the code of generated by the compilator allow us to see this drop:
...
```
Box is ensentially a forced dynamic allocation. 

If we look at the generate code in intermediary step (MIR or Middle level intermediary Representation):
```rust
fn main() -> () {
    let mut _0: ();
...
We can see that there is an added instruction drop.

In this piece of code, we can also see a move. This way we only have one instruction drop. If we add cloned the value, then two drop would have been
visible.
